            ...
            if ($objKeywords->cssFilter)
            {
                foreach ($html->find($objKeywords->cssSelector) as $match)
                {
                    foreach ($match->find('text') as $text)
                    {
                        ...
                        $limit = -1;
                        if($objKeywords->limitCount)
                        {
                            $limit = $objKeywords->count;
                        }

                        $text->innertext = preg_replace('@' . ($objKeywords->words ? '(\A|[^A-Za-z0-9]{1})' : '') . '(' . str_replace('@', '\@', $strQuery) . ')' . ($objKeywords->words ? '([^A-Za-z0-9]{1}|\Z)' : '') . '@su' . ($objKeywords->casesensitive ? '' : 'i'), $strReplacement, $text->innertext, $limit);
                    }
                }
            }
            else
